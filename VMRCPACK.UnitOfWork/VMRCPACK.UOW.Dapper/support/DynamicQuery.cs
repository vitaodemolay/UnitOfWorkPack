...
namespace VMRCPACK.UOW.Dapper.support
{
    ...
    public sealed class DynamicQuery
    {
        ...
        /// <param name="fieldIdName">The Name of field Id</param>
        /// <returns>A result object with the generated sql and dynamic params.</returns>
        public static QueryResult GetDynamicQuery<T>(string tableName, string fieldIdName, Expression<Func<T, bool>> expression)
        {
            ...
            {
                if (item.PropertyName.ToUpper() == "ID" && !string.IsNullOrEmpty(fieldIdName))
                    item.PropertyName = fieldIdName;
            }
            ...
            for (int i = 0; i < queryProperties.Count(); i++)
            {
                ...
                if (queryProperties.FindAll(f => f.PropertyName == propName).Count > 1)
                    propName += i.ToString();

                if (!string.IsNullOrEmpty(item.LinkingOperator) && i > 0)
                {
                    builder.Append(string.Format("{0} {1} {2} @{3} ", item.LinkingOperator, item.PropertyName,
                                                 item.QueryOperator, propName));
                }
                else
                {
                    builder.Append(string.Format("{0} {1} @{2} ", item.PropertyName, item.QueryOperator, propName));
                }

                expando["@" + propName] = item.PropertyValue;
            }
